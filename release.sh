#!/usr/bin/env bash
#
# Release Preparation Script for Dindoa
#
...
if [[ -f flake.nix ]] && [[ -f go.mod ]]; then
    ...
    if [[ "$DEPS_CHANGED" == "true" ]]; then
        if command -v nix &> /dev/null; then
            ...
            VENDOR_HASH=""
            BUILD_OUTPUT=$(nix build .#dindoa 2>&1 || true)
            ...
            if [[ -n "$VENDOR_HASH" ]] && [[ "$VENDOR_HASH" =~ ^sha256-[A-Za-z0-9+/=]+$ ]]; then
                ...
                print_info "Manual update: Run 'nix build .#dindoa 2>&1 | grep got:' and update flake.nix"
            fi
        else
            ...
            print_info "  nix build .#dindoa 2>&1 | grep 'got:'"
        fi
    fi
fi
echo ""
...
if [[ -f go.mod ]] && command -v go &> /dev/null; then
    ...
    if go build -ldflags "-s -w" -o dindoa.test cmd/dindoa/main.go 2>&1; then
...
echo "    https://github.com/torreirow/dindoa/actions"
echo ""
print_info "  View the release (when complete):"
echo "    https://github.com/torreirow/dindoa/releases/tag/v${NEW_VERSION}"
print_info "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
print_success "Done! 🚀"
echo ""
